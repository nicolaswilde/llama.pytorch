...
class Model:
    def __init__(self, gguf_path: str, max_ctx_len: int = 8192, device: str = "cuda"):
        ...
        self.token_embed_weight = torch.empty(self.n_vocab, self.d, dtype=torch.float16, device=device)                           # [n_vocab, d]

        self.attn_q_weight = torch.empty(self.n_block, self.n_head_q, self.d_k, self.d, dtype=torch.float16, device=device)       # [n_block, n_head_q, d_k, d]
        self.attn_k_weight = torch.empty(self.n_block, self.n_head_kv, self.d_k, self.d, dtype=torch.float16, device=device)      # [n_block, n_head_kv, d_k, d]
        self.attn_v_weight = torch.empty(self.n_block, self.n_head_kv, self.d_v, self.d, dtype=torch.float16, device=device)      # [n_block, n_head_kv, d_v, d]
        self.attn_o_weight = torch.empty(self.n_block, self.d, self.n_head_q * self.d_v, dtype=torch.float16, device=device)      # [n_block, d, n_head_q * d_v]
        self.attn_norm_weight = torch.empty(self.n_block, self.d, dtype=torch.float32, device=device)                             # [n_block, d]

        self.ffn_gate_weight = torch.empty(self.n_block, self.d_ffn, self.d, dtype=torch.float16, device=device)                  # [n_block, d_ffn, d]
        self.ffn_up_weight   = torch.empty(self.n_block, self.d_ffn, self.d, dtype=torch.float16, device=device)                  # [n_block, d_ffn, d]
        self.ffn_down_weight = torch.empty(self.n_block, self.d, self.d_ffn, dtype=torch.float16, device=device)                  # [n_block, d, d_ffn]
        ...
        self.kcache = torch.empty((self.n_block, self.n_head_kv, self.max_ctx_len, self.d_k), dtype=torch.float16, device=device) # [n_block, n_head_kv, max_ctx_len, d_k]
        self.vcache = torch.empty((self.n_block, self.n_head_kv, self.max_ctx_len, self.d_v), dtype=torch.float16, device=device) # [n_block, n_head_kv, max_ctx_len, d_v]
    ...
    def load_tensors(self):
        ...
        self.token_embed_weight.copy_(torch.from_numpy(dequantize(
            token_embed_tensor.data.copy(), token_embed_tensor.tensor_type).astype(np.float16)).view(self.n_vocab, self.d))       # [n_vocab, d]

        # Load attention weights
        for i in range(self.n_block):
            attn_q_tensor = self.get_tensor_by_name(f"blk.{i}.attn_q.weight")
            self.attn_q_weight[i] = torch.from_numpy(dequantize(
                attn_q_tensor.data.copy(), attn_q_tensor.tensor_type).astype(np.float16)).view(self.n_head_q, self.d_k, self.d)   # [n_head_q, d_k, d]

            attn_k_tensor = self.get_tensor_by_name(f"blk.{i}.attn_k.weight")
            self.attn_k_weight[i] = torch.from_numpy(dequantize(
                attn_k_tensor.data.copy(), attn_k_tensor.tensor_type).astype(np.float16)).view(self.n_head_kv, self.d_k, self.d)  # [n_head_kv, d_k, d]

            attn_v_tensor = self.get_tensor_by_name(f"blk.{i}.attn_v.weight")
            self.attn_v_weight[i] = torch.from_numpy(dequantize(
                attn_v_tensor.data.copy(), attn_v_tensor.tensor_type).astype(np.float16)).view(self.n_head_kv, self.d_v, self.d)  # [n_head_kv, d_v, d]

            attn_o_tensor = self.get_tensor_by_name(f"blk.{i}.attn_output.weight")
            self.attn_o_weight[i] = torch.from_numpy(dequantize(
                attn_o_tensor.data.copy(), attn_o_tensor.tensor_type).astype(np.float16)).view(self.d, self.n_head_q * self.d_v)  # [d, n_head_q * d_v]
        ...
        for i in range(self.n_block):
            ...
            self.ffn_gate_weight[i] = torch.from_numpy(dequantize(
                ffn_gate_tensor.data.copy(), ffn_gate_tensor.tensor_type).astype(np.float16)).view(self.d_ffn, self.d)            # [d_ffn, d]

            ffn_up_tensor = self.get_tensor_by_name(f"blk.{i}.ffn_up.weight")
            self.ffn_up_weight[i] = torch.from_numpy(dequantize(
                ffn_up_tensor.data.copy(), ffn_up_tensor.tensor_type).astype(np.float16)).view(self.d_ffn, self.d)                # [d_ffn, d]

            ffn_down_tensor = self.get_tensor_by_name(f"blk.{i}.ffn_down.weight")
            self.ffn_down_weight[i] = torch.from_numpy(dequantize(
                ffn_down_tensor.data.copy(), ffn_down_tensor.tensor_type).astype(np.float16)).view(self.d, self.d_ffn)            # [d, d_ffn]
    ...
    def rmsnorm(self, x: torch.Tensor, weight: torch.Tensor):
        ...
        return ((x_fp32 / norm) * weight).to(torch.float16)
    ...
    def rope(self, x: torch.Tensor, pos: int = 0):
        ...
        return torch.cat((y_left, y_right), dim=-1).to(torch.float16)  # [nh, sl, d_rope]
